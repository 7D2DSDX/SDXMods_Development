...
public class MechAnimationSDX : MonoBehaviour, IAvatarController
{
    ...
    void Awake()
    {
        ...
        try
        {
            ...
            this.anim = this.ModelTransform.GetComponent<Animator>();//FindAnimator(this.ModelTransform);
            if (this.anim == null)
            {
            ...
            Log("Avatar is called: " + this.anim.avatar.name.ToString());
    ...
    private Animator FindAnimator(Transform t)
    {
        ...
        {
            localAnimator = t.GetComponent<Animator>();
            Log("Found Animator: " + localAnimator.name.ToString());
    ...
    public bool IsValidAndPlaying( String strAnimation )
    {
        ...
        Log("Checking For Valid Animation: " + strAnimation);
        if (isValidAnimation(strAnimation))
        {
            Log("Valid Animation: " + strAnimation);
            if (this.anim.GetCurrentAnimatorStateInfo(0).IsName(strAnimation))
            {
                Log("Animation is already playing.");
                return true;
            }
            else
            {
                Log("Animation is not playing.");
                return false;
            }
        }

        Log("Animation is not valid: " + strAnimation);
    ...
    protected void Update()
    {
        ...
        //    if (encroached > 1.0 && !isAlwaysWalk)
        //    {
        //        Log("Runnning!");
        //        // Since the encroached is above 1, we want the zombie to run if need be, to get to the player faster.
        //        PlayAnimation(this.AnimationRun);
